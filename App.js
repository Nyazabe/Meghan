...
const App = () => {
    ...
    const speakText = async (text, buttonElement) => {
        ...
        // Only show loading spinner if a button element is provided
        if (buttonElement) {
        ...
        // Restore original icon if a button element was provided
        if (buttonElement) {
    ...
    const processMessage = async (userMessage, autoPlayAudio = false) => {
        ...
        let payloadParts = [];
        if (userMessage) payloadParts.push({ text: userMessage });
        if (uploadedText) payloadParts.push({ text: uploadedText });
        if (base64Image) {
            payloadParts.push({ 
        ...
        const newUserMessage = { role: "user", parts: payloadParts };
        ...
        const payload = {
            contents: currentChatHistory.slice(1), 
    ...
    useEffect(() => {
        ...
        setChatHistory([
            { role: "user", parts: [{ text: getSocraticSystemPrompt() }] },
            initialMessage
        ]);
    }, [language]);
        ...
        <div className="flex flex-col h-screen bg-gray-100 antialiased">
            ...
            <div className="flex-1 flex flex-col max-w-2xl mx-auto w-full bg-white rounded-lg shadow-xl overflow-hidden md:my-4">
                ...
                <div className="p-4 bg-gray-100 flex items-center border-t border-gray-200 rounded-b-lg">
                    ...
                    <input
                        type="text"
                        className="flex-1 p-3 mx-2 border-2 border-gray-300 rounded-full focus:outline-none focus:border-blue-500 transition-colors"
